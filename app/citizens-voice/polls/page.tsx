...
export default function PollsPage() {
  ...
  const [pollResults, setPollResults] = useState<Record<number, Awaited<ReturnType<typeof fetchPollResults>>>>({});
  ...
  const loadAllPolls = async () => {
    try {
      ...
      let hasMore = true;
      while (hasMore) {
        const data = await fetchPolls(currentPage, 100);
    ...
    <div className="min-h-screen bg-[#f5f0e8]">
      ...
      <main className="relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12 pb-8">
          ...
          <section className={xPollEmbeds.length > 0 ? 'bg-[#fafaf8] rounded-xl border border-gray-200 p-6' : ''}>
            <h2 className="text-xl font-bold text-gray-900 mb-4">Platform Polls</h2>
            {loading ? (
              <div className="text-center py-12">
                <span className="inline-block animate-spin rounded-full h-8 w-8 border-2 border-t-transparent" style={{ borderColor: MAROON }} />
                <p className="mt-4 text-gray-600">Loading polls...</p>
              </div>
            ) : error ? (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-red-800">{error}</p>
                <Button onClick={loadAllPolls} className="mt-4 rounded-md" style={{ backgroundColor: MAROON }}>
                  Try Again
                </Button>
              </div>
            ) : allPolls.length === 0 ? (
                    ...
                    >
                      <div className="bg-gradient-to-br from-[#fafaf8] to-[#f5f0e8] p-6 border-b border-gray-200">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          {poll.featured && (
                            <span className="px-2 py-1 text-white text-xs font-medium rounded" style={{ backgroundColor: MAROON }}>
                              Featured
                            </span>
                          )}
                        </div>
                      ...
                      <div className="p-6">
                                      ...
                                      <div className="flex items-center gap-3">
                                        {poll.is_active && !hasVoted && (
                                          <input
                                            ...
                                            tabIndex={-1}
                                            disabled
                                            className="w-4 h-4 pointer-events-none focus:ring-[#800020]"
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <div className="w-full bg-[#f5f0e8] rounded-full h-2">
                                        <div
                                          ...
                                          style={{ width: `${percentage}%`, backgroundColor: MAROON }}
                                        />
                                      </div>
                                    </div>
                                  )}
                        ...
                        {poll.is_active && !hasVoted && poll.total_votes > 0 && (
                          <Button
                            ...
                            className="w-full mt-4 rounded-md border-[#800020] text-[#800020] hover:bg-[#800020]/10"
