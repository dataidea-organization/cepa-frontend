              ...
              {trivias.map((trivia) => {
                const shareUrl =
                  typeof window !== 'undefined'
                    ? `${window.location.origin}/citizens-voice/trivia/${trivia.id}`
                    : `/citizens-voice/trivia/${trivia.id}`;
                ...
                return (
                  <div
                    key={trivia.id}
                    className="relative rounded-xl border border-gray-200 overflow-hidden shadow-md transition-all hover:shadow-lg hover:border-[#800020]/30 aspect-[4/3] min-h-[240px] group"
                  >
                    <button
                      type="button"
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-[#fafaf8] to-[#f5f0e8] flex items-center justify-center overflow-hidden pointer-events-none">
                      {trivia.image ? (
                        ...
                        <HelpCircle className="w-14 h-14 opacity-40 relative z-0" style={{ color: MAROON }} />
                      )}
                    </div>
                    <div
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-10 p-4 pb-3 pt-16 pointer-events-none">
                      ...
                      <div className="flex items-center justify-between gap-2 mt-2">
                        <span className="text-sm font-medium text-[#a3e635] drop-shadow-md">
                          {trivia.question_count ?? trivia.questions?.length ?? 0} question
                          {(trivia.question_count ?? trivia.questions?.length ?? 0) !== 1 ? 's' : ''} – Play
                        </span>
                        <button
                          type="button"
                          onClick={copyLink}
              ...
              >
                <div className="flex items-center justify-between gap-2 p-4 border-b border-gray-200 flex-shrink-0">
                  <h2
                    ...
                    className="text-lg font-semibold text-gray-900 truncate flex-1 min-w-0"
                  >
                    ...
                    {triviaModalData && (
                      <button
                        ...
                        onClick={() => {
                          const url =
                            typeof window !== 'undefined'
                              ? `${window.location.origin}/citizens-voice/trivia/${triviaModalId}`
                              : `/citizens-voice/trivia/${triviaModalId}`;
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  ...
                  {!triviaModalLoading &&
                    ...
                    (() => {
                          ...
                          <div className="py-6">
                            ...
                            <ul className="space-y-2 mb-6">
                              {questions.map((q, i) => (
                                ...
                                >
                                  ...
                                  <span className="text-sm text-gray-900 line-clamp-2 flex-1">
                                    {q.question_text}
                                  </span>
                                </li>
                              ))}
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 justify-center">
                              <Button
                                ...
                                variant="outline"
                                className="rounded-md border-[#800020] text-[#800020] hover:bg-[#800020]/10"
                              >
                                Play again
                              </Button>
                              <Button
                      ...
                      const hasAnswer =
                        (currentQuestion?.answer_text?.trim() ?? '').length > 0;
                      const correctOption = hasOptions
                        ? currentQuestion?.options.find((opt) => opt.is_correct)
                        : null;
                      const selectedOption =
                        triviaSelectedOptionId && hasOptions
                          ? currentQuestion?.options.find(
                              (opt) => opt.id === triviaSelectedOptionId
                            )
                          : null;
                      ...
                      return (
                        <>
                                ...
                                return (
                                    ...
                                    className={`w-full text-left p-3 rounded-lg border-2 transition-all ${
                                      isSelected
                                        ...
                                        : showCorrect && isCorrectOption
                                          ? 'border-green-500 bg-green-50'
                                          : 'border-gray-200 hover:border-[#800020]/50 hover:bg-[#f5f0e8]'
                                  ...
                                  >
                                    <div className="flex items-start gap-3">
                                      ...
                                      <span
                                        className={`flex-1 text-sm text-gray-900 ${isSelected && !isCorrectOption ? 'line-through opacity-60' : ''}`}
                                      >
                                        {option.text}
                                      </span>
                                    </div>
                          ...
                          {triviaSelectedOptionId !== null && hasOptions && (
                            <div
                              className={`p-3 rounded-lg mb-4 ${isCorrect ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
                            >
                              <div className="flex items-start gap-3">
                                {isCorrect ? (
                          ...
                          {!hasOptions && hasAnswer && (
                            <div className="mt-4">
                              {!triviaRevealed ? (
                                ...
                                <div className="p-3 bg-[#f5f0e8] rounded-lg border border-[#800020]/20 text-sm text-gray-900 whitespace-pre-wrap">
                                  {currentQuestion?.answer_text}
                                </div>
                              )}
                            </div>
                ...
                {!triviaModalLoading &&
                  !triviaModalError &&
                  triviaModalData &&
                  (triviaModalData.questions?.length ?? 0) > 0 &&
                  !triviaShowingResults && (
                    <div className="flex items-center justify-between p-4 border-t border-gray-200 bg-[#fafaf8] flex-shrink-0">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setTriviaCurrentIndex((i) => Math.max(0, i - 1))
                        }
                      ...
                      {triviaCurrentIndex ===
                        (triviaModalData.questions?.length ?? 1) - 1 &&
                      triviaSelectedOptionId !== null ? (
                        <Button
                          type="button"
                          size="sm"
                          onClick={() => setTriviaShowingResults(true)}
                          className="gap-1 rounded-md text-white"
                          style={{ backgroundColor: MAROON }}
                        >
                          See results
                        </Button>
